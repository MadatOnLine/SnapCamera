...
public class FocusOverlayManager {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case RESET_TOUCH_FOCUS: {
                    // westalgo add begin
                    if (mIsBokehMode) {
                        resetFocusUI();//just close focus ui
                    } else {
                        cancelAutoFocus();
                    }
                    // westalgo add end
    ...
    // westalgo add begin
    public PointF getFocusPosition(Point touchPoint, int rotation) {
        PointF p = new PointF(0.5f, 0.5f);
        if (touchPoint != null && mPreviewRect != null) {
            // preview ratio
            if (rotation == 90) {
                p.x = (float) (touchPoint.y - mPreviewRect.top) / mPreviewRect.height();
                p.y = (float) (mPreviewRect.right - touchPoint.x) / mPreviewRect.width();
    ...
    public void setBokehMode(boolean value) {
        mIsBokehMode = value;
    }
    // westalgo add end
}
